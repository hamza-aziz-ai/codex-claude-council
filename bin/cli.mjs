...
async function main() {
  ...
  const pairs = single
    ...
    : { codex_model: values['codex-model'], codex_effort: values['codex-effort'], claude_model: values['claude-model'], claude_effort: values['claude-effort'] };
  const wrong = single
    ? ['codex-model', 'codex-effort', 'claude-model', 'claude-effort'].filter(key => values[key] !== undefined)
  ...
  console.log(await invoke(tool, question, options));
  return 0;
}
