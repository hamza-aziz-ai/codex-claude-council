...
- **Neither model can change your files.** Codex runs `codex exec` in its **read-only sandbox**, enforced by the operating system, ignoring your `~/.codex/config.toml` and `.rules` files (so no plugins, hooks or MCP servers load). Claude Code runs with `--restricted` (no user, project or local settings, so no hooks, plugins or allow rules from them), no MCP servers and no skills, and with `--permission-mode dontAsk`, so it can use only its read tools (Read, Grep, Glob) and a list of read-only git commands; anything else is refused, including git options that would write a file (`--output`) or read one outside the project (such as `blame --contents`).
