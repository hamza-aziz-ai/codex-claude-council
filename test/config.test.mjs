...
test('packaged defaults apply when there is no user config', () => withUserConfig(undefined, () => {
  ...
  assert.equal(config.tool_wait_seconds, 50, 'each tool call returns within a 60 s host limit');
...
test('broken config files give a clear error', () => {
  ...
  withUserConfig({ tool_wait_seconds: 0 }, () => assert.equal(loadConfig().tool_wait_seconds, 0));
  withUserConfig({ tool_wait_seconds: -1 }, () => assert.throws(() => loadConfig(), /tool_wait_seconds/));
